...
IMPORTANT RULES:
1. For categories related to KPI queries (KPI Extraction, Comparitive Analysis, Trend Analysis), always set requires_sql to true
...
5. **Missing Location Rule:** If the question requests a KPI that requires a location (e.g., 'orders', 'atp') but does *not* specify any location (no CFC, no spoke, no mention of 'network' or 'all'), classify it as 'Ambiguous Questions'.
6. When in doubt about classification, choose the closest match and lower the confidence score
7. Return ONLY the JSON with no additional text, explanations, or markdown formatting
"""
